...
//			sendEvent name: "verboseTrace", value: "doRequest>token= ${data.auth.access_token}"
		...
		if (exceptionCheck.contains("Unauthorized")) {
			...
			sendEvent name: "verboseTrace", value:
				"doRequest>and continue processing with refresh_tokens() call"
		}    
		refresh_tokens()        
...
def refreshLocalAuthToken() {	
	...
	def buffer_time_refresh=1  // set a 1 min. buffer time before re-attempting refresh 
	...
	if ((state?.lastRefreshTimestamp) && (state?.lastRefreshTimestamp > time_check_for_refresh)) {
		if (settings.trace) {
			log.debug "refresh_tokens>thermostatId = ${thermostatId},time_check_for_refresh (${time_check_for_refresh}) < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
			sendEvent name: "verboseTrace", value:
				"refresh_tokens>thermostatId = ${thermostatId},time_check_for_refresh (${time_check_for_refresh} < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
		}
		return false
	}
	...
	if (settings.trace) {                        
		...
		sendEvent name: "verboseTrace", value:
			"refreshLocalAuthToken>about to refresh auth token"
	}
		...
		if (resp.status == REFRESH_SUCCESS_CODE) {
			...
			if (resp.data) {
				...
				if (settings.trace) {                        
					...
					sendEvent name: "verboseTrace", value:
						("refreshLocalAuthToken>new authexptime = ${authexptime}")
					log.debug("refreshLocalAuthToken>new access token = ${data.auth.access_token}")
					sendEvent name: "verboseTrace", value:
						("refreshLocalAuthToken>new access token = ${data.auth.access_token}")
					log.debug("refreshLocalAuthToken>new refresh token = ${data.auth.refresh_token}")
					sendEvent name: "verboseTrace", value:
							("refreshLocalAuthToken>new refresh token = ${data.auth.refresh_token}")
				}  
			...
			sendEvent name: "verboseTrace", value:
				"refreshLocalAuthToken>refresh tokens failed ${resp.status} : ${resp.status.code}"
...
//	if device created by initialSetup (Service Manager)
	...
	if (data.auth?.thermostatId) {
		...
		if (!isTokenExpired()) {
			if (settings.trace) {
				log.debug "refresh_tokens>child tokens no longer expired"
			}
			return true            
		} else {
			if (settings.trace) {
				sendEvent name: "verboseTrace", value:
					"refresh_tokens>warning: child tokens still expired after refreshChildTokens() call"
				log.warn "refresh_tokens>child tokens still expired after refreshChildTokens() call"
			}
			return false
		}
		return true        
    }  /* end if device created by initialSetup */
